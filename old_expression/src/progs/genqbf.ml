...
let fprint_circuit ch qphi : unit =
  ...
  (* let generator = *)
    if      !Args.anonymous     then QBF.generate_quantified_formula_anon desc k
    else if !Args.is_exists_num then QBF.generate_quantified_ES_formula desc k n_exists
    else if !Args.is_forall_num then QBF.generate_quantified_AS_formula desc k n_forall
    else                             QBF.generate_quantified_formula desc k
  (* in *)
  (* generator desc k  *)
...
let parse_I_file () : formula =
  if !Args.is_initial_file_A then
    let ch = Stdlib.open_in !Args.initial_file_A in
     Parser.parse ch (EParser.letclause ELexer.norm)
  else
